...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        part_tr = torch.utils.data.random_split(train_dataset, [2000, len(train_dataset)-2000])[0]
...
def save_checkpoint(state, epoch):
    path = os.path.join('./weights', 'b6')
    os.makedirs(path, exist_ok=True)
    filename=os.path.join(path,'{}_500_checkpoint.pth.tar'.format(epoch))
